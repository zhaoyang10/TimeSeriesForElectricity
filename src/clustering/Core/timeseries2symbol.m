...
% Code was modified U-Shapelet project speed-up
...
dataLen = size(data, 1);
pointers         = zeros(dataLen + 1, 1); %[];                                                  % Initialize pointers,
symbolic_data = zeros(dataLen +1, n); %zeros(1,n);                                             % Initialize symbolic_data with a void string, it will be removed later.
lastPointer = 1;
for i = 1 : dataLen                                       
    sub_section = data(i, :);
    ...
    if ~all(current_string == symbolic_data(lastPointer,:))     % If the string differs from its leftmost neighbor...
        lastPointer = lastPointer + 1;
        symbolic_data(lastPointer,:)    = current_string;       % ... add it to the set...
        pointers(lastPointer)           = i;                    % ... and add a new pointer.
    end;    
end;
...
switch alphabet_size
        ...
        otherwise disp('Error! alphabet_size is too large');           
end;
