...
for i = be : en
    ...
    if mod(i, block_size) == 0
        toc;
        j = i - block_size + 1;
        display(['historydata_node_10_trans_', num2str(j), '_to_', num2str(i) ,'treated successfully!']);
    end
end
j = j + block_size;
if j ~= len
    toc;
    display(['~/datasets/hangzhoujiahe/treated_data/historydata_node_10_trans_', num2str(j), '_to_', num2str(j + len - 1) ,' treated successfully!']);
end

save(['~/datasets/hangzhoujiahe/treated_data/historydata_node_10_trans_', num2str(be), '_to_', num2str(en), '.mat'], 'data_mat', '-v7.3');
toc;
display(['~/datasets/hangzhoujiahe/treated_data/historydata_node_10_trans_', num2str(be), '_to_', num2str(en), '.mat saved successfully!']);
...
for i = 1 : len_nodeid
    ...
    save([save_path, 'historydata_node_10_trans_', nodeid_uni{i}, '_', num2str(be), '_to_', num2str(en), '.mat'], 'data_sub', 'data_nodeid', '-v7.3');
    toc;
    display([save_path, 'historydata_node_10_trans_', nodeid_uni{i}, '_', num2str(be), '_to_', num2str(en), '.mat saved!']);
end
...
for i1 = 1 : len_nodeid
    load([save_path, 'historydata_node_10_trans_', nodeid_uni{i1}, '_', num2str(be), '_to_', num2str(en), '.mat']);
    ...
    save([save_path, 'historydata_node_10_trans_', data_nodeid, ...
        '_sensor_parted_', num2str(be), '_to_', num2str(en), '.mat'], 'sensor_name', 'sensor_data', 'sensor_reserve', ...
        'sensor_time', 'alarm', 'data_nodeid', '-v7.3');
    toc;
    display([save_path, 'historydata_node_10_trans_', data_nodeid, '_sensor_parted_', num2str(be), '_to_', num2str(en), '.mat saved successfully!']);
end

for il = 1 : len_nodeid  
    load([save_path, 'historydata_node_10_trans_', nodeid_uni{il}, ...
        '_sensor_parted_', num2str(be), '_to_', num2str(en), '.mat']);
    sensor_time_total = [];
    ...
    save([save_path,'historydata_node_10_trans_', data_nodeid, '_time_completed_', num2str(be), '_to_', num2str(en), '.mat'], 'data_nodeid', 'sensor_data_complete', 'sensor_time_complete', 'alarm_complete', 'sensor_name', '-v7.3');
    display([save_path,'historydata_node_10_trans_', data_nodeid, '_time_completed_', num2str(be), '_to_', num2str(en), '.mat saved successfully!']);
